...
class Location extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
      const { location , getLocation, locationObject} = this.props;
  ...
  //     if (this.props.locationObject !== prevProps.locationObject){
  //         console.log("not equal")
  //         this.props.getSimilarLocations(this.props.locationObject)
  //     }
  // }
  ...
  checkLandmark(locationObject) {
    if(
      locationObject &&
      locationObject.landmarks &&
      locationObject.landmarks.length > 0
      ) {
      return locationObject.landmarks.map((landmark, index) => {
        return (
          <Badge color="warning" key={index}>
            {landmark || ""}
          </Badge>
        );
      });
              ...
              {success ? (
                <Link to="/">
                  ...
                  <div>
                    ...
                    >
                      Pay with Bank/Cheque
                  </Button>
                    {this.props.currency === "NGN" ? (
                      <CheckoutPaystack
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <p style={{ color: "#000", fontSize: 18, marginTop: 15 }}>{location ? !location.traffic ? `Traffic: ${0}` : `Traffic: ${location.traffic.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}` : 0}</p>

                  <div className={classes.price}>
                    <p style={{ marginTop: 15, fontSize: 18, color: "#000"}}> <span style={{color: '#000'}}>Size: </span>{` ${
                      location.count ? location.size : 0
                    }%`}</p>
                  </div>
    ...
    if (!discountedPrice.checked) {
      return (
        <div>
          ...
          <div>
            <h4 style={{ fontSize: 9, color: infoColor[1] }}>{`${location.pricingOption ? "per " + location.pricingOption.name : ""
              }`}</h4>
          </div>
        </div>
      );
                  ...
                  <GridContainer>
                    <GridItem md={12} sm={12}>
                      <ImageGallery
                        ...
                        startIndex={0}
                        items={this.cal(
                          locationObject.resizedImages ? (locationObject.resizedImages || []) : (locationObject.images || []),
                  ...
                  <GridContainer>
                    <GridItem md={12} sm={12}>
                      <h2
                        ...
                        style={{ textTransform: "capitalize", color:'#0a24a7' }}
                      >
                        {locationObject.name
                      ...
                      {!showBookingDetails && (
                        <GridContainer className={classes.pullRight}>
                          <Button
                            ...
                            onClick={() => locationShowBookingDetails()}
                          >
                            BOOK ADSPACE
                          </Button>
                        </GridContainer>
...
)(withStyles(productStyle)(Location));
